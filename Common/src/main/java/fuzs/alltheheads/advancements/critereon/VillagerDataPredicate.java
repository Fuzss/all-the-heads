...
public record VillagerDataPredicate(Optional<VillagerType> type,
    ...
    public boolean matches(Entity entity, ServerLevel level, @Nullable Vec3 position) {
        if (!(entity instanceof VillagerDataHolder dataHolder)) {
            return false;
        } else if (this.type.isPresent() && dataHolder.getVillagerData().getType() != this.type.get()) {
            return false;
        } else if (this.profession.isPresent()
                && dataHolder.getVillagerData().getProfession() != this.profession.get()) {
            return false;
        } else if (this.level.isPresent() && dataHolder.getVillagerData().getLevel() != this.level.get()) {
            return false;
        } else {
            return true;
        }
    ...
    public static VillagerDataPredicate hasData(VillagerType type, VillagerProfession profession) {
