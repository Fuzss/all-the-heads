...
public class AllTheHeadsClient implements ClientModConstructor {
    ...
    public void onRegisterBuiltInBlockModels(BuiltInBlockModelsContext context) {
        // The context for the head type is stored on the block entity, which is not available here.
        // Our default fallback is the player head, which is also used elsewhere.
        context.registerModelFactory(ModRegistry.MOB_HEAD_BLOCK.value(),
                BuiltInBlockModels.createMobHead(SkullBlock.Types.PLAYER));
        context.registerModelFactory(ModRegistry.MOB_WALL_HEAD_BLOCK.value(),
                BuiltInBlockModels.createMobWallHead(SkullBlock.Types.PLAYER));
    }
}
